...
> В настоящее время применение **null** считается плохой практикой и его не рекомендуется
> использовать в коде на Scala.
...
У чисел есть и другие методы, например, методы преобразования типов:
...
Логический тип Boolean имеет два значения *false* (ложь) и *true* (истина). Методы "логическое И" (&& и &) и "логическое ИЛИ" (|| и |) записываются в инфиксной форме, а "логическое НЕ" (!) - в префиксной:
...
Для точной арифметики можно использовать типы BigInt (целые) и BigDecimal (вещественные числа):
